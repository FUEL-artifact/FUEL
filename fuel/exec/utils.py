...
def is_equal_tensor(
    ...
    if lib == "pytorch":
        ...
        ):  # if x and y are both empty tensor FIXME@: In the comment, we use `both` but why we use `or` in code?
            return True, 0
...
def inductor_numerical_verification(
    ...
    if eager_output.numel() == 0 or inductor_output.numel() == 0 or fp64.numel() == 0:
        return (
            True,
            0,
        )  # TODO@: Add the judgement on one of the output tensors is empty
