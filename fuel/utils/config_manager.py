...
def ensure_config_file_exists(config_path: str, config_type: str, lib: str = None) -> str:
    """
    ...
    if config_type in ["gen_prompt", "als_prompt"] and lib:
        ...
        if not os.path.exists(config_path):
            logger.warning(f"Config directory {config_path} does not exist, creating...")
            os.makedirs(config_path, exist_ok=True)
        
        # Then check the specific library file
        full_config_path = os.path.join(config_path, f"{lib}.yaml")
        if not os.path.exists(full_config_path):
            logger.warning(f"Config file {full_config_path} does not exist, attempting to download...")
            _download_config_file(config_type, full_config_path, lib)
    else:
        # For single config files
        if not os.path.exists(config_path):
            logger.warning(f"Config file {config_path} does not exist, attempting to download...")
...
def _download_config_file(config_type: str, save_path: str, lib: str = None):
    ...
    try:
        ...
        raise ConnectionError(f"Network error: Failed to download configuration file from {url}. Please check your network connection and the URL.") from e
    except Exception as e:
        logger.error(f"An unexpected error occurred while trying to download config file from {url}: {e}")
...
def validate_all_configs(lib: str, gen_prompt_config: str, 
    ...
    try:
        ...
        als_prompt_base = ensure_config_file_exists(als_prompt_config, "als_prompt", lib)
        validated_configs["ALS_PROMPT_CONFIG"] = als_prompt_base + f"/{lib}.yaml"
        
        # Validate heuristic config
        validated_configs["HEURISTIC_CONFIG"] = ensure_config_file_exists(heuristic_config, "heuristic")
